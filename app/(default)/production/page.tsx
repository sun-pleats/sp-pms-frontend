...
const ProductionPage = () => {
  ...
  const { kioskFilters, setKioskFilters, setSectionOptions, sectionOptions, logStyleBundle, reset } = useKiosk();
  ...
  const initData = () => {
    fetchSectionOptions()
      .then((options) => setSectionOptions(options))
    ...
    <>
      <div className="surface-0 flex align-items-center flex-row p-5">
        <div className="flex align-items-center gap-2">
          <FormDropdown
            value={kioskFilters.selectedSection}
            onChange={(option) => setKioskFilters({ ...kioskFilters, selectedSection: option.value })}
            label="Section"
