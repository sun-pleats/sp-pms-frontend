...
const EditOperatorPage = ({ params }: EditOperatorPageProps) => {
  ...
  const getOperator = useCallback(async () => {
    const op = (await OperatorService.getOperator(params?.id)).data as Operator;
    setOperator({
      id: op.id,
      name: op.name,
      section_id: op.section_id,
      deterministic_output: op.deterministic_output,
      operator_processes: op.operator_processes?.map((r) => r.process) ?? []
    });
  }, [params?.id]);
